...
function AboutUs() {
  ...
  useEffect(() => {
    const retrieveData = async () => {
      const activeResults = await sqljs(
        `SELECT * FROM IndividualDetail WHERE Status = 'Active'`
      );
      const legacyResults = await sqljs(
      ...
      setLegacyTop4(
        formattedLegacyData.filter((member) => member.Division === "Top4")
      );
      setLegacyDirectors(
        formattedLegacyData.filter((member) => member.Division === "Director")
      );
    };
    retrieveData();
  }, []);
